...
/* Los arcos de las últimas 2 partidas. El arco es lo que define la FORMA de
   la carrera (subida sostenida, travesía, montaña rusa): si se repite, dos
   partidas se sienten iguales aunque las cartas sean otras. */
export function arcosRecientes() {
  const v = cargarVitrina();
  return new Set(v.ultimosArcos || []);
}
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        ultimas: v.ultimas || [],
        // ...y los últimos 2 arcos: es lo que hace que dos partidas se SIENTAN
        // iguales aunque las cartas cambien
        ultimosArcos: v.ultimosArcos || [],
        // SPEC v4 §3 — aprendizajes desbloqueados
        aprendizajes: v.aprendizajes || [],
      };
    }
  } catch { /* sin persistencia */ }
  return { copas: [], arquetipos: [], partidas: 0, setup: null, ultimas: [], ultimosArcos: [], aprendizajes: [] };
}
...
export function actualizarVitrina(gs) {
  ...
  const ultimosArcos = [gs.arco, ...(v.ultimosArcos || [])].slice(0, 2);
  const nueva = {
    copas: [...copas],
