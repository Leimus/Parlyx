...
import { pickCard, pickEmergency, planMacro, planSlots, cardById, matchTePaso, POST_EXIT_CARD, DIA_DESPUES_CARD, sillaDe, modoSilla } from "./deck.js";
...
/* El arco define la FORMA de la carrera. `evitarArcos` son los de las
   últimas 2 partidas: se despriorizan (no se prohíben, como las cartas del
   §2.3), para que jugar tres veces seguidas no dé tres veces la misma
   silueta. Medido sin esto: 19.7% de repetición contra la anterior. */
function sortearArco(rs, evitarArcos = new Set()) {
  const pesos = arcosJson.arcos.map((a) => ({ a, w: a.prob * (evitarArcos.has(a.id) ? 0.25 : 1) }));
  const tot = pesos.reduce((s, x) => s + x.w, 0);
  let r = rs() * tot;
...
export function createGame(seedStr, setup, evitar = new Set(), evitarArcos = new Set()) {
  ...
  const arco = sortearArco(rsArco, evitarArcos);
  ...
  const gs = {
    ...
    // SPEC v4 §2: la seed elige una candidata por slot al empezar la partida
    slotPlan: planSlots(seedStr, setup, arco.id, evitar),
    slotsVistos: [],
    used: new Set(),
...
/* ---------- SPEC v4 §1.3 — VARIANZA SILENCIOSA ----------
   Una carta sin porcentaje daba SIEMPRE exactamente lo mismo: se memoriza
   en dos partidas. Ahora todo efecto OCULTO se resuelve con ±35% alrededor
   de su base, y en el 12% de los casos sale un resultado de cola (bastante
   mejor o bastante peor, con su propio texto de reveal).

   Los COSTOS declarados (rw, eq, pat) NO varían jamás: el jugador los vio
   antes de elegir, son un contrato. Varía solo lo que no le prometimos.
   El jugador nunca ve el dado; ve que la misma decisión no siempre rinde
   igual — como en la vida. */
const VAR_BANDA = 0.35;
const PROB_COLA = 0.12;
const COLA_FUERTE = 1.75;
const COLA_FLOJA = 0.35;

/* La cola es una DIRECCIÓN (a favor o en contra del jugador), no una
   magnitud: si fuera magnitud, una cola "buena" sobre un efecto malo lo
   haría peor todavía. Por eso el factor se elige según el signo de cada
   efecto — a favor agranda lo bueno y achica lo malo, y al revés. */
function tirarVarianza(gs) {
  const r = rng(gs.g);
  if (r < PROB_COLA / 2) return { cola: "peor" };
  if (r < PROB_COLA) return { cola: "mejor" };
  return { f: 1 + (rng(gs.g) * 2 - 1) * VAR_BANDA, cola: null };
}

/* Factor efectivo para un efecto según su signo y la cola que salió. */
function factorPara(v, tirada) {
  if (!tirada.cola) return tirada.f;
  const bueno = v >= 0;
  const aFavor = tirada.cola === "mejor";
  return bueno === aFavor ? COLA_FUERTE : COLA_FLOJA;
}

/* Escala un efecto aditivo conservando su signo y un mínimo de 1 punto:
   una varianza que convierte un +2 en +0 no se siente como suerte, se
   siente como que la carta no hizo nada. */
const escalaAditivo = (v, tirada) => {
  const s = Math.sign(v);
  return s * Math.max(1, Math.round(Math.abs(v) * factorPara(v, tirada)));
};
/* Escala un multiplicador alrededor de 1: ×1.30 con f=0.5 → ×1.15.
   El "signo" de un multiplicador es si está por encima o por debajo de 1. */
const escalaMul = (m, tirada) => Math.max(0.05, 1 + (m - 1) * factorPara(m - 1, tirada));

function applyEfectos(gs, ef, year, ctx, { variar = false } = {}) {
  if (!ef) return;
  const g = gs.g;
  // La tirada es UNA por decisión: todos los efectos ocultos de la misma
  // opción se mueven juntos (si no, se promedian entre sí y no se siente).
  const v = variar ? tirarVarianza(gs) : { f: 1, cola: null };
  if (v.cola) gs.cola = v.cola;
  if (ef.ovr) g.ovr = clampOvr(g, g.ovr + escalaAditivo(ef.ovr, v));
  if (ef.tend) g.tendQueue.push({ v: escalaAditivo(ef.tend, v), left: 2 });
  if (ef.tendLenta) g.tendQueue.push({ v: escalaAditivo(ef.tendLenta, v), left: 2, delay: 1 });
  if (ef.rw) g.cash += ef.rw * burnMensual(g); // COSTO declarado: no varía
  if (ef.eq) g.eq = Math.max(1, Math.min(100, g.eq + ef.eq)); // COSTO declarado
  if (ef.arrMul) g.arr *= escalaMul(ef.arrMul, v);
  if (ef.pat) g.pat += ef.pat; // COSTO/cobro declarado
  if (ef.patMul) g.pat = Math.max(0, g.pat * ef.patMul);
  if (ef.patPct) g.pat = Math.max(0, g.pat * (1 + ef.patPct));
  if (ef.valMul) gs.pendingValMul *= escalaMul(ef.valMul, v);
  if (ef.multMul) gs.pendingValMul *= escalaMul(ef.multMul, v);
  if (ef.techoDelta) g.techo = Math.max(60, Math.min(99, g.techo + escalaAditivo(ef.techoDelta, v)));
...
/* ---------- SPEC v4 §1.4 — EL REVEAL ES EL APRENDIZAJE ----------
   Después de elegir, el jugador ve en una línea qué pasó de verdad. Es la
   mitad del contenido: es acá donde aprende el sistema que le escondimos
   antes de decidir. Reglas:
     · Los efectos de largo plazo (lo que era TEND) NO se etiquetan con
       jerga: como mucho se insinúan ("esto se va a notar más adelante").
     · Los resultados de cola (§1.3) tienen su propio texto: el jugador
       tiene que sentir que esa vez salió distinto.
     · Si la opción trae `reveal` escrito a mano, gana ese. */
const fotoDe = (g) => ({ ovr: g.ovr, arr: g.arr, cash: g.cash, pat: g.pat, eq: g.eq });

/* La cola se dice como MATIZ al final, no como veredicto: si abriera la
   línea, chocaría con lo que describe el resto ("salió peor… quedó mejor"). */
/* Las dos listas hablan de lo que la jugada PROMETÍA, no de si el
   resultado fue bueno o malo: así componen con cualquier descripción sin
   contradecirla ("quedó mejor parada… y encima todo en contra" era un bug). */
const REVEAL_COLA_MEJOR = [
  "Rindió más de lo que prometía.",
  "Y esta vez acompañó más de lo esperable.",
  "Bastante más de lo que la jugada prometía.",
];
const REVEAL_COLA_PEOR = [
  "Rindió menos de lo que prometía.",
  "Podría haber rendido bastante más.",
  "Bastante menos de lo que la jugada prometía.",
];

function construirReveal(gs, op, antes) {
  const g = gs.g;
  const dOvr = g.ovr - antes.ovr;
  const dArr = antes.arr > 0 ? g.arr / antes.arr - 1 : 0;
  const largo = (op.efectos?.tend || 0) + (op.efectos?.tendLenta || 0);
  const partes = [];

  if (op.reveal) partes.push(op.reveal);
  else {
    if (dArr > 0.02) partes.push("Las ventas se movieron para arriba.");
    else if (dArr < -0.02) partes.push("Las ventas se resintieron.");
    if (dOvr >= 2) partes.push("Tu marca quedó bastante mejor parada.");
    else if (dOvr > 0) partes.push("Tu marca quedó un poco mejor parada.");
    else if (dOvr <= -2) partes.push("Te dejó golpeado.");
    else if (dOvr < 0) partes.push("Te costó algo de terreno.");
    if (!partes.length && !largo) partes.push("Por ahora no se movió nada. Habrá que ver.");
  }
  // El largo plazo se insinúa, nunca se etiqueta (§1.4). Si no pasó nada hoy
  // pero sí quedó sembrado algo, la línea habla SOLO del futuro: decir "no se
  // movió nada" y "se va a notar" en la misma línea se pisa.
  if (largo > 0) partes.push(partes.length ? "Esto se va a notar más adelante." : "Hoy no se movió nada, pero esto se va a sentir en los años que vienen.");
  else if (largo < 0) partes.push(partes.length ? "Algo de esto te lo vas a encontrar más adelante." : "Hoy no se notó, pero algo de esto te lo vas a encontrar más adelante.");

  if (gs.cola) {
    const pool = gs.cola === "mejor" ? REVEAL_COLA_MEJOR : REVEAL_COLA_PEOR;
    partes.push(pool[Math.floor(rng(g) * pool.length)]);
  }
  const bueno = dOvr > 0 || dArr > 0.02 || (gs.cola === "mejor" && dOvr >= 0 && dArr >= 0);
  return { good: bueno, critico: false, reveal: true, cola: gs.cola, texto: partes.join(" ") };
}
...
export function chooseOption(gs, opId) {
  ...
  const antes = fotoDe(g);
  // §1.3: la varianza silenciosa corre en las cartas determinísticas. Las de
  // apuesta ya traen su propio azar declarado: no se les suma otro por atrás.
  applyEfectos(gs, op.efectos, year, ctx, { variar: !op.apuesta });
  ...
  if (op.apuesta) {
    ...
    // §1.4: EL REVEAL ES EL APRENDIZAJE. Toda decisión muestra qué pasó, no
    // solo las apuestas: es donde el jugador aprende cómo funciona el juego.
    result = construirReveal(gs, op, antes);
  }
  gs.result = result;
