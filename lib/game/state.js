...
import { pickCard, pickEmergency, planMacro, cardById, matchTePaso, POST_EXIT_CARD, DIA_DESPUES_CARD } from "./deck.js";
...
export function createGame(seedStr, setup) {
  ...
  const gs = {
    ...
    acquihire: false,
    ipoTecnico: false,
    parlyx: null, // { desde, convos, ventas, horas } al activar (PRD v1.0 §4)
    parlyxBeat: null,
    convosPerdidas: 0,
...
function applySpecial(gs, special, year, ctx) {
  ...
  switch (special) {
    ...
    case "playa":
      ...
      break;
    case "parlyxActivar":
      gs.parlyx = { desde: year, convos: 0, ventas: 0, horas: 0 };
...
const MODO_ROW = {
  playa: { name: "— Playa —", emoji: "🏝", color: "#22D3EE", etapa: "Retiro" },
  corpo: { name: "El gigante regional", emoji: "📦", color: "#F0B90B", etapa: "Corpo" },
  austero: { name: "— Vida tranquila —", emoji: "🌿", color: "#8B93A3", etapa: "Retiro" },
};

function pushRow(gs, year, prevArr, prevVal) {
  const g = gs.g;
  const alt = MODO_ROW[gs.mode];
...
function playaTurn(gs, year) {
  ...
  if (gs.mode === "corpo") {
    ...
    g.pat += 400000 + g.ovr * 6000;
    g.ovr = clampOvr(g, g.ovr + (rng(g) < 0.5 ? 1 : 0));
  } else if (gs.mode === "austero") {
    g.pat += 60000; // laburo tranquilo: se vive, se ahorra poco
  } else {
...
export function advanceTurn(gs) {
  ...
  if (gs.mode === "corpo" || gs.mode === "austero") {
    // Capítulo post-quiebra (§7): los años corren solos hasta 2026
    while (g.ti < TURN_YEARS.length) playaTurn(gs, TURN_YEARS[g.ti]);
    return finishGame(gs);
  }
  ...
  if (gs.mode === "playa") {
    ...
    if (gs.parlyx) {
      const convos = Math.max(40, Math.round(g.arr / 2500));
      ...
      gs.parlyxBeat = `🤖 Parlyx detectó ${convos} conversaciones con intención de compra → ${ventas} ventas → ${horas}h recuperadas`;
    } else {
      gs.parlyxBeat = null;
...
export function finishGame(gs) {
  const g = gs.g;
  // Los años restantes corren solos (retiro o capítulo post-quiebra)
  while (!g.dead && g.ti < TURN_YEARS.length && (gs.retired || gs.pendingPostExit || gs.mode === "corpo" || gs.mode === "austero")) {
    if (gs.pendingPostExit) {
      gs.retired = true;
      gs.mode = "playa";
      gs.pendingPostExit = false;
