...
import { NOMBRES, ETAPAS } from "./meta.js";
...
export function chooseOption(gs, opId) {
  ...
  let result = null;
  if (op.apuesta) {
    const win = rng(g) < op.apuesta.p;
    const rama = win ? op.apuesta.gana : op.apuesta.pierde;
    applyEfectos(gs, rama.efectos, year, ctx);
    if (rama.goto) goto = rama.goto;
    result = { good: win, texto: rama.texto || (win ? "Salió bien" : "Salió mal") };
  }
  gs.result = result;
...
export function advanceTurn(gs) {
  ...
  if (gs.mode === "playa") {
    ...
    const m = simulateTrienio(g, null); // ECONOMÍA REAL — motor intacto, incrementa g.ti
