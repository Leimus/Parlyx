...
function eligible(card, gs) {
  ...
  if (card.id === "EC-24" || card.id === "EC-25") return false; // beats anclados (PRD v1.0 §4)
...
export function pickCard(gs) {
  ...
  // Beats anclados por estructura (PRD v1.0 §4): 2020 = Parlyx desbloqueado;
  // 2023 = "El tiempo que recuperaste" solo si lo activaste.
  if (year === 2020 && gs.mode === "founder" && !gs.used.has("EC-24")) {
    gs.used.add("EC-24");
    return cardById("EC-24");
  }
