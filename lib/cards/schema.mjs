...
/* Las tres sillas (SPEC v3 §2): founder = A · corpo = B (incluye "austero",
   que es la misma silla con menos escala) · playa = C, el Ángel.
   "ejecutivo" queda por el bloque EJ que sigue apagado hasta F3. */
export const MODOS = ["founder", "ejecutivo", "playa", "corpo"];
export const TIPOS = ["T1", "T2", "T3", "T4", "T5", "T6", "T7", "T8", "T9"];
export const BLOQUES = [
  "macro", "ejecutivo", "generales", "emergencia", "ecommerce",
  "hq", "equipo", "financiamiento", "playa", "color", "slots",
];
/* Conteo por bloque. El pivote v1.0 reemplazó la capa de verticales tech
   (16) por la capa e-commerce (25). La SPEC v3 §2 suma la silla B (6 cartas
   de corpo, incl. el beat 2020) y engorda la silla C del Ángel (4 cartas):
   deck de 122 cartas. */
export const CARTAS_POR_BLOQUE = {
  macro: 15, ejecutivo: 12, generales: 20, emergencia: 5, ecommerce: 32,
  hq: 8, equipo: 8, financiamiento: 5, playa: 12, color: 5, slots: 40,
};
...
const Opcion = z
  .object({
    ...
    /* Texto EXACTO de efectos del .md (después de la flecha →). Es el
       contrato con la fuente de contenido: NO se toca. */
    raw: z.string().min(1),
    /* SPEC v4 §1 — la línea que ve el jugador: costo duro literal + una
       frase de CARÁCTER. Nunca un número de resultado. Si falta, la UI
       cae a humanizar(raw) (que sí puede spoilear: por eso el checker
       scripts/check-informacion.mjs exige `linea` donde haga falta). */
    linea: z.string().min(1).optional(),
    efectos: Efectos.optional(),
    /* SPEC v4 §1.4 — reveal escrito a mano para esta opción. Si falta, el
       juego sintetiza uno con lo que realmente pasó. */
...
export const Card = z
  .object({
    id: z.string().regex(/^(M|EJ|G|E|EC|H|T|F|P|C|S)-\d{2}$/),
    ...
    /* SPEC v4 §2.3 — tono de la candidata, para que el ARCO sesgue el sorteo
       del slot: "crecimiento" (jugadas de empuje), "resistencia" (jugadas de
       aguante) o "neutro". Solo sesga; nunca prohíbe. */
    tono: z.enum(["crecimiento", "resistencia", "neutro"]).optional(),
